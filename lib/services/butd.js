...
/**
 * Provides a friendly event driven API to butd in Node.js. Manages starting and
 * stopping butd as a child process for application support, as well as connecting
 * to multiple butd processes for server infrastructure. Results are cached in an
...
Dash.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'butd/' + name, 'total:', this.subscriptions[name].length);
};
...
Dash.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'butd/' + name, 'total:', this.subscriptions[name].length);
};
...
Dash.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'butd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Dash.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'butd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Dash.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in butd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in butd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in butd config options');
...
Dash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that butd is undergoing a reindex. ' +
...
Dash.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('butd/hashblock', message.toString('hex'));
    }
  }
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('butd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Dash.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('butd/rawtransaction', message.toString('hex'));
    }
...
Dash.prototype._zmqTransactionLockHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('butd/transactionlock', message.toString('hex'));
    }
  }
};
...
Dash.prototype._stopSpawnedDash = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/butd.pid';
...
Dash.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedDash(function(err) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn butd.'));
      }
...
Dash.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to butd.'));
    }
...
Dash.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addTxlockToTx(tx,result) {
    // 2019-06-15: still not working very well, provide fallback logic (auto InstantSend must have <= 4 inputs and mempool <= 10% full)
    if (tx.instantlock || tx.instantlock_internal || tx.chainlock || tx.inputs.length <= 4)
      tx.txlock = true;
    if (tx.txlock)
      return;
    //wtf, don't kill it: tx.txlock = false;
    //not longer required: if (tx.feeSatoshis >= Dash.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new dashcore.Transaction(result.hex);
      var hash = dashcore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.butd.zmqKnownTransactionLocks.get(id)) {
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('butd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('butd process did not exit'));
      }
