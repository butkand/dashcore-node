...
var DashService = proxyquire('../../lib/services/butd', {
  fs: {
...
describe('Dash Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var butd = new DashService(baseConfig);
      should.exist(butd);
    });
    it('will create an instance without `new`', function() {
      var butd = DashService(baseConfig);
      should.exist(butd);
    });
    it('will init caches', function() {
      var butd = new DashService(baseConfig);
      should.exist(butd.utxosCache);
      should.exist(butd.txidsCache);
      should.exist(butd.balanceCache);
      should.exist(butd.summaryCache);
      should.exist(butd.transactionDetailedCache);
      should.exist(butd.smartnodeListCache);

      should.exist(butd.transactionCache);
      should.exist(butd.rawTransactionCache);
      should.exist(butd.blockCache);
      should.exist(butd.rawBlockCache);
      should.exist(butd.blockHeaderCache);
      should.exist(butd.zmqKnownTransactions);
      should.exist(butd.zmqKnownBlocks);
      should.exist(butd.lastTip);
      should.exist(butd.lastTipTimeout);
    });
    it('will init clients', function() {
      var butd = new DashService(baseConfig);
      butd.nodes.should.deep.equal([]);
      butd.nodesIndex.should.equal(0);
      butd.nodes.push({client: sinon.stub()});
      should.exist(butd.client);
    });
    it('will set subscriptions', function() {
      var butd = new DashService(baseConfig);
      butd.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: [],
        transactionlock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var butd = new DashService(baseConfig);
      butd._initDefaults({transactionConcurrency: 10});
      butd.transactionConcurrency.should.equal(10);
      butd._initDefaults({});
      butd.transactionConcurrency.should.equal(5);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var butd = new DashService(baseConfig);
      var methods = butd.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var butd = new DashService(baseConfig);
      var events = butd.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('butd/rawtransaction');
      events[0].scope.should.equal(butd);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('butd/transactionlock');
      events[1].scope.should.equal(butd);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('butd/hashblock');
      events[2].scope.should.equal(butd);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('butd/addresstxid');
      events[3].scope.should.equal(butd);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var butd = new DashService(baseConfig);
      butd.subscribe = sinon.stub();
      butd.unsubscribe = sinon.stub();
      var events = butd.getPublishEvents();

      events[0].subscribe('test');
      butd.subscribe.args[0][0].should.equal('rawtransaction');
      butd.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      butd.unsubscribe.args[0][0].should.equal('rawtransaction');
      butd.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      butd.subscribe.args[1][0].should.equal('transactionlock');
      butd.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      butd.unsubscribe.args[1][0].should.equal('transactionlock');
      butd.unsubscribe.args[1][1].should.equal('test');

      events[2].subscribe('test');
      butd.subscribe.args[2][0].should.equal('hashblock');
      butd.subscribe.args[2][1].should.equal('test');

      events[2].unsubscribe('test');
      butd.unsubscribe.args[2][0].should.equal('hashblock');
      butd.unsubscribe.args[2][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var butd = new DashService(baseConfig);
      var emitter = {};
      butd.subscribe('hashblock', emitter);
      butd.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      butd.subscribe('rawtransaction', emitter2);
      butd.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      butd.subscribe('hashblock', emitter1);
      butd.subscribe('hashblock', emitter2);
      butd.subscribe('hashblock', emitter3);
      butd.subscribe('hashblock', emitter4);
      butd.subscribe('hashblock', emitter5);
      butd.subscriptions.hashblock.length.should.equal(5);

      butd.unsubscribe('hashblock', emitter3);
      butd.subscriptions.hashblock.length.should.equal(4);
      butd.subscriptions.hashblock[0].should.equal(emitter1);
      butd.subscriptions.hashblock[1].should.equal(emitter2);
      butd.subscriptions.hashblock[2].should.equal(emitter4);
      butd.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var butd = new DashService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      butd.subscriptions.hashblock= [emitter1];
      butd.unsubscribe('hashblock', emitter3);
      butd.subscriptions.hashblock.length.should.equal(1);
      butd.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      butd.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(butd.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var butd = new DashService(baseConfig);
      var emitter = new EventEmitter();
      butd.subscribeAddress(emitter, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will handle multiple address subscribers', function() {
      var butd = new DashService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      butd.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      butd.subscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      butd.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
      butd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      butd.unsubscribeAddress(emitter1);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      butd.unsubscribeAddress(emitter1, ['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs']);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter2];
      butd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      butd.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      butd.unsubscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.not.exist(butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      butd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      sinon.spy(butd, 'unsubscribeAddressAll');
      butd.unsubscribeAddress(emitter1);
      butd.unsubscribeAddressAll.callCount.should.equal(1);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      butd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      butd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      butd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      butd.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'] = [emitter1];
      butd.unsubscribeAddress(emitter1);
      butd.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      butd.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
      butd.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(butd.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var butd = new DashService(baseConfig);
      var config = butd._getDefaultConfig();
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a dash.conf file', function() {
      var TestDash = proxyquire('../../lib/services/butd', {
        fs: {
          readFileSync: readFileSync,
      ...
      var butd = new TestDash(baseConfig);
      butd.options.spawn.datadir = '/tmp/.dash';
      var node = {};
      butd._loadSpawnConfiguration(node);
      should.exist(butd.spawn.config);
      butd.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var butd = new TestDash(config);
      butd.options.spawn.datadir = './data';
      var node = {};
      butd._loadSpawnConfiguration(node);
      butd.options.spawn.datadir.should.equal('/tmp/.dashcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      ...
      var butd = new TestDash(baseConfig);
      (function() {
        butd._loadSpawnConfiguration({datadir: './test'});
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the dash.conf file', function() {
      ...
      var node = {};
      butd._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        butd.transactionDetailedCache.set(keys[i], {});
        butd.utxosCache.set(keys[i], {});
        butd.txidsCache.set(keys[i], {});
        butd.balanceCache.set(keys[i], {});
        butd.summaryCache.set(keys[i], {});
      }
      butd._resetCaches();
      should.equal(butd.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(butd.utxosCache.get(keys[0]), undefined);
      should.equal(butd.txidsCache.get(keys[0]), undefined);
      should.equal(butd.balanceCache.get(keys[0]), undefined);
      should.equal(butd.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      butd._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        butd.nodes[0].client.getInfo.callCount.should.equal(1);
        butd.nodes[1].client.getInfo.callCount.should.equal(1);
        butd.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert butd-rpc object into JavaScript error', function() {
      var butd = new DashService(baseConfig);
      var error = butd._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      butd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      butd.nodes.push({
        client: {
      ...
      butd._initChain(function() {
        log.info.callCount.should.equal(1);
        butd.getRawBlock.callCount.should.equal(1);
        butd.getRawBlock.args[0][0].should.equal('genesishash');
        butd.height.should.equal(5000);
        butd.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      butd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      butd._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    ...
    it('will get default rpc port for livenet', function() {
      ...
      var butd = new DashService(config);
      butd._getDefaultConf().rpcport.should.equal(9998);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var butd = new DashService(config);
      butd._getDefaultConf().rpcport.should.equal(19998);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var butd = new DashService(config);
      should.equal(butd._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var butd = new DashService(config);
      butd._getNetworkConfigPath().should.equal('testnet3/dash.conf');
    });
    it('will get default rpc port for regtest', function() {
      dashcore.Networks.enableRegtest();
      var config = {
      ...
      var butd = new DashService(config);
      butd._getNetworkConfigPath().should.equal('regtest/dash.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var butd = new DashService(baseConfig);
      butd.node.network = dashcore.Networks.testnet;
      butd._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var butd = new DashService(baseConfig);
      butd.node.network = dashcore.Networks.testnet;
      dashcore.Networks.enableRegtest();
      butd._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var butd = new DashService(baseConfig);
      butd.node.network = dashcore.Networks.livenet;
      dashcore.Networks.enableRegtest();
      should.equal(butd._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      butd._rapidProtectedUpdateTip = sinon.stub();
      butd.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      butd._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      butd._rapidProtectedUpdateTip = sinon.stub();
      butd._zmqBlockHandler(node, message);
      butd._rapidProtectedUpdateTip.callCount.should.equal(1);
      butd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      butd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      butd._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      butd.subscriptions.hashblock.push(emitter);
      emitter.on('butd/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        butd._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub();
      ...
      butd._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      butd.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      butd.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      butd.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub();
      butd._resetCaches = sinon.stub();
      butd.on('tip', function(height) {
        butd._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        butd.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      butd.on('tip', function() {
        butd.syncPercentage.callCount.should.equal(0);
        done();
      });
      butd._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using dashcore.Transaction', function() {
      ...
      tx.sign(privkey);
      var addresses = butd._getAddressesFromTransaction(tx);
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      butd._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      butd.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('butd/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      butd._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      butd.subscriptions.rawtransaction.push(emitter);
      emitter.on('butd/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      butd._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      butd.subscriptions.rawtransaction.push(emitter);
      emitter.on('butd/rawtransaction', function() {
        done();
      });
      var node = {};
      butd._zmqTransactionHandler(node, expectedBuffer);
  ...
  describe('#_zmqTransactionLockHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      butd.subscriptions.transactionlock.push(emitter);
      emitter.on('butd/transactionlock', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      butd._zmqTransactionLockHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      var butd = new DashService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      butd.subscriptions.transactionlock.push(emitter);
      emitter.on('butd/transactionlock', function() {
        done();
      });
      var node = {};
      butd._zmqTransactionLockHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var butd = new DashService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      butd.on('txlock', function(buffer) {
        buffer.should.be.instanceof(Buffer);
        buffer.toString('hex').should.equal(expectedBuffer.toString('hex'));
        done();
      });
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var butd = new DashService(baseConfig);
      butd._updateTip = sinon.stub();
      butd._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      butd.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      butd._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        butd._updateTip.callCount.should.equal(11);
        butd._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var butd = new DashService(baseConfig);
      butd._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        butd._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var butd = new DashService(baseConfig);
      butd._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      butd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        butd._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      butd._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        butd._zmqBlockHandler.callCount.should.equal(0);
        butd._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var butd = new DashService(baseConfig);
      var node = {};
      butd._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      butd._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      butd._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      butd._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestDashService = proxyquire('../../lib/services/butd', {
        fs: {
          readFile: readFile
        }
      });
      var butd = new TestDashService(baseConfig);
      butd.spawnStopTime = 1;
      butd._process = {};
      butd._process.kill = sinon.stub();
      butd._stopSpawnedDash(function(err) {
        if (err) {
          return done(err);
        }
        butd._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      butd._process.kill = sinon.stub().throws(error2);
      butd._stopSpawnedDash(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var butd = new DashService(baseConfig);
      butd._loadSpawnConfiguration = sinon.stub();
      butd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      butd._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give error from stopSpawnedDash', function() {
      var butd = new DashService(baseConfig);
      butd._loadSpawnConfiguration = sinon.stub();
      butd._stopSpawnedDash = sinon.stub().callsArgWith(0, new Error('test'));
      butd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var butd = new TestDashService(config);
      butd.spawn = {};
      butd._loadSpawnConfiguration = sinon.stub();
      butd._stopSpawnedDash = sinon.stub().callsArgWith(0, null);
      butd.node.stopping = true;
      butd._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      butd._loadSpawnConfiguration = sinon.stub();
      butd.spawn = {};
      butd.spawn.exec = 'testexec';
      butd.spawn.configPath = 'testdir/dash.conf';
      butd.spawn.datadir = 'testdir';
      butd.spawn.config = {};
      butd.spawn.config.rpcport = 20001;
      butd.spawn.config.rpcuser = 'dash';
      butd.spawn.config.rpcpassword = 'password';
      butd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      butd.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';

      butd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      butd._initZmqSubSocket = sinon.stub();
      butd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      butd._checkReindex = sinon.stub().callsArgWith(1, null);
      butd._spawnChildProcess(function(err, node) {
        ...
        butd._loadTipFromNode.callCount.should.equal(1);
        butd._initZmqSubSocket.callCount.should.equal(1);
        should.exist(butd._initZmqSubSocket.args[0][0].client);
        butd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        butd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(butd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn butd spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestDashService = proxyquire('../../lib/services/butd', {
        fs: {
      ...
      butd._loadSpawnConfiguration = sinon.stub();
      butd.spawn = {};
      butd.spawn.exec = 'butd';
      butd.spawn.datadir = '/tmp/dash';
      butd.spawn.configPath = '/tmp/dash/dash.conf';
      butd.spawn.config = {};
      butd.spawnRestartTime = 1;
      butd._loadTipFromNode = sinon.stub().callsArg(1);
      butd._initZmqSubSocket = sinon.stub();
      butd._checkReindex = sinon.stub().callsArg(1);
      butd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      butd._stopSpawnedDash = sinon.stub().callsArg(0);
      sinon.spy(butd, '_spawnChildProcess');
      butd._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            butd._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      butd._spawnChildProcess(function(err) {
        ...
        butd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        butd.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn butd spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestDashService = proxyquire('../../lib/services/butd', {
        fs: {
      ...
      butd._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            butd._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      butd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      butd.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';
      butd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      butd._spawnChildProcess(function(err) {
        butd._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      butd._initZmqSubSocket = sinon.stub();
      butd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      butd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var butd = new DashService(config);
      butd.node.stopping = true;
      butd.startRetryInterval = 100;
      butd._loadTipFromNode = sinon.stub();
      butd._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        butd._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      butd.startRetryInterval = 1;
      var config = {};
      butd._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        butd._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      butd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      butd._connectProcess(config, function(err, node) {
        should.not.exist(err);
        butd._loadTipFromNode.callCount.should.equal(1);
        butd._initZmqSubSocket.callCount.should.equal(1);
        butd._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error from connectProcess', function(done) {
      var butd = new DashService(baseConfig);
      butd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      butd.options = {
        connect: [
          {}
        ]
      ...
      butd.start(function(err) {
        butd._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var butd = new DashService(baseConfig);
      var node = {};
      butd._initChain = sinon.stub().callsArg(0);
      butd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      butd.options = {
        spawn: {}
      };
      butd.start(function(err) {
        should.not.exist(err);
        butd.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      butd._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      butd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      butd.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      butd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      butd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      butd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var butd = new DashService(baseConfig);
      butd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      butd.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      butd.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      butd.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var butd = new DashService(baseConfig);
      var args = butd._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var butd = new DashService(baseConfig);
      var args = butd._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      butd.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      butd.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        butd.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      butd.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      butd.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = butd._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = butd._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = butd._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      butd._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        butd._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var butd = new DashService(baseConfig);
      butd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      butd.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      butd.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      butd.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      butd.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        butd.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var butd = new DashService(baseConfig);
      butd.height = 10;
      var confirmations = butd._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var butd = new DashService(baseConfig);
      var result = {};
      butd._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var butd = new DashService(baseConfig);
      var result = {};
      butd._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var butd = new DashService(baseConfig);
      var result = {};
      butd._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var butd = new DashService(baseConfig);
      var result = {};
      butd._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = butd._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var butd = new DashService(baseConfig);
      butd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      butd.height = 300;
      var addresses = {};
      butd._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      butd._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var butd = new DashService(baseConfig);
      butd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      butd._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from dashcore addresses', function() {
      ...
      var butd = new DashService(baseConfig);
      var strings = butd._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var butd = new DashService(baseConfig);
      (function() {
        butd._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var butd = new DashService(baseConfig);
      butd.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var butd = new DashService(baseConfig);
      butd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      butd.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var butd = new DashService(baseConfig);
      butd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      butd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      butd.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var butd = new DashService(baseConfig);
      butd.maxAddressesQuery = 100;
      butd.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var butd = new DashService(baseConfig);
      butd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      butd.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var butd = new DashService(baseConfig);
      butd._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      butd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      butd.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      butd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      butd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      butd.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      butd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      butd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      butd.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(butd, '_paginate');
      butd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      butd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      butd.getAddressSummary(address, options, function(err, summary) {
        butd._paginate.callCount.should.equal(1);
        butd._paginate.args[0][1].should.equal(0);
        butd._paginate.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      butd.getAddressSummary(address, options, function(err, summary) {
        ...
        butd.getAddressTxids.callCount.should.equal(1);
        butd.getAddressBalance.callCount.should.equal(1);
        butd.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          butd.getAddressTxids.callCount.should.equal(1);
          butd.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      butd.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginate', function(done) {
      ...
      butd._paginate = sinon.stub().throws(new Error('test'));
      var address = '7oK6xjGeVK5YCT5dpqzNXGUag1bQadPAyT';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      butd.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      butd.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      butd.nodesIndex = 2;
      butd.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      butd.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will getblock as dashcore object from height', function(done) {
      ...
      butd.getBlock(0, function(err, block) {
        should.not.exist(err);
    ...
    it('will getblock as dashcore object', function(done) {
      ...
      butd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      butd.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      butd.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      butd.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      butd.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      butd.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      butd.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      butd.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#getBlockHeaders', function(){
      ...
      it('will gave error from getBlockHash', function(){
          ...
          butd.getBlockHeaders(10, function(err) {
              err.should.be.instanceof(Error);
          });
      });
      it('it will give rpc error from client getblockheaders', function() {
          ...
          butd.getBlockHeaders(blockhash, function(err){
              err.should.be.instanceof(Error);
          });
      });
      it("will get an array of block headers", function(){
          ...
          butd.getBlockHeaders(_blockHash, function(err, blockHeader){
              should.not.exist(err);
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      butd._maybeGetBlockHash('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      butd._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      butd._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var butd = new DashService(baseConfig);
      butd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      butd.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      butd.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      butd.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      butd.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    ...
    it('will give rpc error', function() {
      ...
      butd.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      butd.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      butd.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = dashcore.Transaction();
      (function() {
        butd.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      butd.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      butd.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      butd.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      butd.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      butd.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      butd.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      butd.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      butd.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      butd.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      butd.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var butd = new DashService(baseConfig);
      butd.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      butd.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#govObject', function() {
    it('will call client gobject list and give result', function(done) {
        ...
        butd.govObjectList({type: 1}, function(err, result) {
            if (err) {
                return done(err);
            }
            should.exist(result);
    ...
    it('will call client gobject get and give result', function(done) {
      ...
      butd.govObjectHash('4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00', function(err, result) {
        if (err) {
          return done(err);
        }
	...
	describe('#sporksList', function(){
		it('will call client sporks and give result', function(done){
			...
			butd.getSpork(function(err, SporkList) {
				if (err) {
					return done(err);
				}
				...
				sporks['SPORK_8_SMARTNODE_PAYMENT_ENFORCEMENT'].should.equal(0);
				sporks['SPORK_9_SUPERBLOCKS_ENABLED'].should.equal(0);
				sporks['SPORK_10_SMARTNODE_PAY_UPDATED_NODES'].should.equal(0);
  ...
  describe('#getMNList', function(){
    it('will call client smartnode list and give result', function(done){
	    var butd = new DashService(baseConfig);
	    butd.isSynced = function(callback) { return callback(null, true) };
	    butd.nodes.push({
		    client: {
	    ...
	    butd.getMNList(function(err, MNList) {
		    if (err) {
			    return done(err);
		    }
    ...
    it('will return error if one of nodes not synced yet', function(done){
      var butd = new DashService(baseConfig);
      butd.isSynced = function(callback) { return callback(null, false) };
      butd.nodes.push({
        client: {
          smartnodelist: function(type, cb){
            switch (type){
    ...
    it('will return error if checking synced state of nodes failed', function(done){
      var butd = new DashService(baseConfig);
      butd.isSynced = function(callback) { return callback(new Error('Failed')) };
      butd.nodes.push({
        client: {
          smartnodelist: function(type, cb){
            switch (type){
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      butd.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      butd.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var butd = new DashService(baseConfig);
      butd.spawn = {};
      butd.spawn.process = new EventEmitter();
      butd.spawn.process.kill = sinon.stub();
      butd.stop(done);
      butd.spawn.process.kill.callCount.should.equal(1);
      butd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      butd.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      butd.spawn.process.kill.callCount.should.equal(1);
      butd.spawn.process.kill.args[0][0].should.equal('SIGINT');
      butd.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var butd = new DashService(baseConfig);
      butd.shutdownTimeout = 300;
      butd.spawn = {};
      butd.spawn.process = new EventEmitter();
      butd.spawn.process.kill = sinon.stub();
      butd.stop(function(err) {
